...
"""
Utils.
Source: https://github.com/MHX1203/py/blob/master/utils/layer_utils.py

Created on Sat Jun 22 17:40:29 2019
@author: Nick Ma
"""
from __future__ import absolute_import

from tensorflow.python.keras import (
    backend as K, activations, initializers, regularizers, constraints
)
...
def ranked_probability_score(y_true, y_pred):
    ...
    RPS = (1 / (r - 1)) * SUM(i = 1, r - 1, SUM (j = 1, j, y_pred[j] - y_true[j]) ** 2)
    @author: Juan Beleño
    '''
    r = 3
    #y_pred = tf.argmax(y_pred, axis=0)
    #y_pred = tf.one_hot(y_pred, r)
    rps = 1 / (r - 1)
    rps_outer = 0
    for i in range(r - 1):
        rps_inner = 0
        for j in range(i):
            rps_inner = rps_inner + (y_pred[j] - y_true[j])
        rps_outer = rps_outer + rps_inner*rps_inner
    rps = rps * rps_outer
    return rps
...
class AttentionLSTMCell(Layer):
    ...
    def build(self, input_shape):
        ...
        # BUG FIX: casting to int because in python > 3.5 is float
